...
def generate_acf_pacf_image_and_stats(file_path, lags, column_index):
    ...
    # Статистика
    test_output = "Параметры ACF/PACF:\n"
    ...
    fig.update_layout(
        title='ACF и PACF анализ с тестами',
        height=1050,
        width=1100,
        showlegend=True
    )

    # Сохранить в памяти, затем в base64
    buf = io.BytesIO()
...
# === Запуск без if __name__ == "__main__" ===
if len(sys.argv) < 4:
    print("Usage: python acfPacfGraphPhoto.py <data_file_path> <lags> <column_index>")
    sys.exit(1)
...
try:
    generate_acf_pacf_image_and_stats(data_file_path, lags, column_index)
except Exception as e:
    print("PYTHON_ERROR:", e)
    traceback.print_exc()
    sys.exit(120)
